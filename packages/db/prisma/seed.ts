...
import { infoLayerDefs, criteriaLayerDefs } from './seed/map_layers';
...
function createUpsert(layerCreate: MapLayerUpsert['create']): MapLayerUpsert {
  return {
    where: { layerId: layerCreate.layerId },
    create: layerCreate,
...
async function upsertMapLayers() {
  ...
  for (const layerDef of infoLayerDefs) {
    ...
    console.log('  ', layerDef.layerId);
    await prisma.mapLayer.upsert(upsert);
  }

  console.info(`Upserting ${criteriaLayerDefs.length} criteria map layers`);
  for (const layerDef of criteriaLayerDefs) {
    const upsert = createUpsert(layerDef);
    console.log('  ', layerDef.layerId);
    await prisma.mapLayer.upsert(upsert);
  }
