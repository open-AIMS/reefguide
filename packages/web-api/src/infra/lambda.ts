...
async function getSecret(secretId: string): Promise<string> {
  ...
  try {
    ...
    const secretString = response.data.SecretString || response.data.secretString;
    console.log('Secret string type:', typeof secretString);
    console.log('Secret string length:', secretString ? secretString.length : 0);
...
async function getJsonSecret<T = any>(secretId: string): Promise<T> {
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      console.error('JSON parsing failed. First 50 chars of input:', secret?.substring(0, 50));
    }
...
function exportToEnv(obj: Record<string, string>): void {
  ...
  try {
    ...
    for (const [key, value] of Object.entries(obj)) {
      console.log(`Exporting key: ${key}, value length: ${String(value).length}`);
      process.env[key] = String(value);
      exportedCount++;
    }

    console.log(`Successfully exported ${exportedCount} variables to process.env`);
...
exports.handler = async (event: any, context: any) => {
  ...
  if (handler) {
    ...
    try {
      ...
      if ([secretArn, workerCredsArn, managerCredsArn, adminCredsArn].some(v => !v)) {
      ...
      const managerCreds = await getJsonSecret<CredsInterface>(managerCredsArn!);
