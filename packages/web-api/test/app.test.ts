...
describe('API', () => {
  ...
  describe('Cache Management', () => {
    ...
    beforeEach(async () => {
      // Create a job and complete it successfully to have cached results
      const job = await prisma.job.create({
        data: {
          type: JobType.TEST,
          status: JobStatus.SUCCEEDED,
          user_id: user1Id,
          input_payload: { id: 12345 },
          hash: await new JobService().generateJobHash({
            payload: { id: 12345 },
            jobType: 'TEST'
          })
      ...
      const assignment = await prisma.jobAssignment.create({
        data: {
          ...
          ecs_task_arn: 'arn:aws:ecs:test:success',
          ecs_cluster_arn: 'arn:aws:ecs:cluster:test:success',
