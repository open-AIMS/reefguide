...
export class ReefGuideAPI extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ReefGuideAPIProps) {
    ...
    const reefGuideTaskDfn = new ecs.FargateTaskDefinition(this, 'reefguide-task-dfn', {
    ...
    const reefGuideContainerDfn = reefGuideTaskDfn.addContainer('reefguide-container-dfn', {
    ...
    if (props.config.memoryAlerting) {
      ...
      alertTopic.addSubscription(new subscriptions.EmailSubscription(alertConfig.emailAddress));
      ...
      const avgMemoryAlarm = new cloudwatch.Alarm(this, 'AvgMemoryUtilizationAlarm', {
      ...
      const maxMemoryAlarm = new cloudwatch.Alarm(this, 'MaxMemoryUtilizationAlarm', {
    ...
    if (props.config.autoScaling.enabled) {
      ...
      scaling.scaleOnCpuUtilization('CpuScaling', {
        ...
        scaleInCooldown: Duration.seconds(props.config.autoScaling.scaleInCooldown),
        scaleOutCooldown: Duration.seconds(props.config.autoScaling.scaleOutCooldown)
      });

      // Configure memory utilization based auto scaling
      scaling.scaleOnMemoryUtilization('MemoryScaling', {
        targetUtilizationPercent: props.config.autoScaling.targetMemoryUtilization,
        scaleInCooldown: Duration.seconds(props.config.autoScaling.scaleInCooldown),
        scaleOutCooldown: Duration.seconds(props.config.autoScaling.scaleOutCooldown)
      });
    }
